...
from test_framework.blocktools import (create_block, create_coinbase, create_transaction, create_block_with_txns)
...
class ExampleTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        for i in range(100):
            ...
            block = create_block(self.tip, coinbase_tx, self.block_time)
            block.solve()
        ...
        for i in range(100):
            ...
            outputValue = int(input_txn.vout[0].nValue / 2)
            txn = create_transaction(input_txn, 0, b'', outputValue)

            block = create_block_with_txns(self.tip, [coinbase_tx, txn], self.block_time)
            block.solve()
            block_message = msg_block(block)
            # Send message is used to send a P2P message to the node over our NodeConn connection
            node0.send_message(block_message)
        ...
        self.log.info(
            "Wait for node1 to reach current tip (height 201) using RPC")
        self.nodes[1].waitforblockheight(201)
